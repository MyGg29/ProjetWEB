		...
		if(this.stars[i][1] < this.magnitude){
			...
			if(this.isVisible(p.el) && !isNaN(p.x) && !this.isPointBad(p)){
				...
				if(this.showstarlabels && this.starnames[this.stars[i][0]]) {
					this.drawLabel(p.x,p.y,d,"",this.htmlDecode(this.starnames[this.stars[i][0]]));
					this.addPointer({
						'ra': this.stars[i][2]*this.r2d,
...
VirtualSky.prototype.drawPlanets = function(){
	...
	for(p = 0 ; p < this.planets.length ; p++){
		...
		//On ajoute aussi un pointer vers la page de la plannete
		this.addPointer({
			'ra': ra,
			'dec': dec,
